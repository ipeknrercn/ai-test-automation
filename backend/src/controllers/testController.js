...
class TestController {
  ...
  // body: { userPrompt? } — verilmezse orijinal prompt kullanılır
  async rerunTest(req, res) {
    try {
      const { userPrompt } = req.body || {};
      const result = await testService.rerunTest(req.params.id, userPrompt);
      res.json({ success: true, data: result });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  async deleteTest(req, res) {
    try {
      const result = await testService.deleteTestRun(req.params.id);
      res.json({ success: true, data: result });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
